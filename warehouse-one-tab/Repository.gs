...
function setupRepository_(spreadsheet) {
  ...
  migrateSchemaSheetNames_(spreadsheet);
...
function ensureRepositorySchemaCurrent_() {
  migrateSchemaSheetNamesSafely_();
...
/**
 * Keep the supplied Items/Logs/Users contract without splitting a workbook
 * that was initialized by the earlier hardened release under Arabic names.
 * All conflicts are detected before the first rename, so a workbook that
 * contains both naming sets fails closed instead of silently reading one set.
 */
function migrateSchemaSheetNamesSafely_() {
  var spreadsheet = getBoundSpreadsheet_();
  var needsMigration = Object.keys(REPOSITORY_SCHEMA_).some(function (key) {
    var schema = REPOSITORY_SCHEMA_[key];
    if (!schema.legacyNames || !schema.legacyNames.length) return false;
    return !spreadsheet.getSheetByName(schema.name) && schema.legacyNames.some(function (name) { return !!spreadsheet.getSheetByName(name); });
  });
  assertNoSchemaSheetNameConflicts_(spreadsheet);
...
function assertNoSchemaSheetNameConflicts_(spreadsheet) {
  Object.keys(REPOSITORY_SCHEMA_).forEach(function (key) {
    var schema = REPOSITORY_SCHEMA_[key];
    var canonical = spreadsheet.getSheetByName(schema.name);
    var legacy = (schema.legacyNames || []).filter(function (name) { return !!spreadsheet.getSheetByName(name); });
    if ((canonical && legacy.length) || legacy.length > 1) {
      throw new WarehouseError_('SCHEMA_SHEET_NAME_CONFLICT', 'توجد أوراق متعارضة لنفس البيانات: ' + [schema.name].concat(legacy).join(' / ') + '. أوقف الاستخدام وادمج البيانات يدوياً قبل المتابعة.', { canonical: schema.name, legacy: legacy });
    }
  });
}

function migrateSchemaSheetNames_(spreadsheet) {
  assertNoSchemaSheetNameConflicts_(spreadsheet);
  Object.keys(REPOSITORY_SCHEMA_).forEach(function (key) {
    var schema = REPOSITORY_SCHEMA_[key];
    if (spreadsheet.getSheetByName(schema.name)) return;
    var legacy = (schema.legacyNames || []).map(function (name) { return spreadsheet.getSheetByName(name); }).filter(Boolean);
    if (legacy.length === 1) legacy[0].setName(schema.name);
  });
}
...
function getSchemaSheet_(schemaKey) {
  ...
  if (!sheet) {
    var legacy = (schema.legacyNames || []).map(function (name) { return spreadsheet.getSheetByName(name); }).filter(Boolean);
    sheet = legacy.length === 1 ? legacy[0] : null;
  }
